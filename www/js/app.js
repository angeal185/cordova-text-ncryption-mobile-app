        ...
        var roundsArray = [, , , , [, , , , 10, , 12, ,
            14
        ], , [, , , , 12, , 12, , 14], , [, , , , 14, , 14, , 14]];
        var shiftOffsets = [, , , , [, 1, 2, 3], , [, 1, 2,
            3
        ], , [, 1, 3, 4]];
        ...
        function byteArrayToHex(byteArray) {
            var result = "";
            if (!byteArray) return;
            for (var i = 0; i < byteArray.length; i++) result += ((byteArray[i] < 16) ? "0" : "") + byteArray[i].toString(16);
            return result;
        }
        ...
        function hexToByteArray(hexString) {
            ...
            for (var i = 0; i < hexString.length; i += 2) byteArray[Math.floor(i / 2)] = parseInt(hexString.slice(i, i + 2), 16);
            return byteArray;
        }
        ...
        function keyFromEntropy() {
            var i,
                k = new Array(32);
            if (edlen == 0) {
                alert("Entropy vector void at call to keyFromEntropy.");
            }
            md5_init();
            for (i = 0; i < edlen; i += 2) {
